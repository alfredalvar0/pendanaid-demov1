<div class="modal fade" id="konfirmasiHapus" role="dialog">
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
        <div class="col-md-offset-1 col-md-10 col-md-offset-1 well">
	      <h3 style="display:block; text-align:center;">Apakah Anda Yakin Hapus Data Ini ?</h3>

	      <div class="col-md-6">
	        <button class="form-control btn btn-primary hapus-dataBank"> <i class="glyphicon glyphicon-ok-sign"></i> Ya</button>
	      </div>
	      <div class="col-md-6">
	        <button class="form-control btn btn-danger" data-dismiss="modal"> <i class="glyphicon glyphicon-remove-sign"></i> Tidak</button>
	      </div>

	    </div>
    </div>
  </div>
</div>
<script>

  	window.onload = function(){
  		tampilData();
  		<?php
			if ($this->session->flashdata('msg') != '') {
				echo "effect_msg();";
			}
		?>
  	}

  	function refresh() {
		$('#list-data').DataTable({
			"responsive":true
		});
	}

	function effect_msg_form() {
		// $('.form-msg').hide();
		$('.form-msg').show(1000);
		setTimeout(function() { $('.form-msg').fadeOut(1000); }, 3000);
	}

	function effect_msg() {
		// $('.msg').hide();
		$('.msg').show(1000);
		setTimeout(function() { $('.msg').fadeOut(1000); }, 3000);
	}

	// Film
	function tampilData() {
		$.get('<?php echo base_url('Article/tampil_category'); ?>', function(data) {
			$('#list-data').dataTable().fnDestroy();
			$('#data-category-article').html(data);
			refresh();
		});
	}

	var id_bank;
	$(document).on("click", ".konfirmasiHapus-bank", function() {
		id_bank = $(this).attr("data-id");
	})
	$(document).on("click", ".hapus-dataBank", function() {
		var id = id_bank;

		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Toc/delete'); ?>",
			data: "id=" +id
		})
		// alert(id)
		.done(function(data) {
			$('#konfirmasiHapus').modal('hide');
			tampilBank();
			$('.msg').html(data);
			effect_msg();
		})
	})

	$('#formAddModal').on('show.bs.modal', function (event) {
	  var button = $(event.relatedTarget);
	  // var recipient = button.data('whatever');
	  var modal = $(this);
	  modal.find('.modal-title').text(button.data('label'));
	  modal.find('#category').val(button.data('category'));
	  modal.find('#form_category').attr('action', button.data('action'));
	})

</script>
