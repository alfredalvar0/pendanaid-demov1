...
{
  return $this->db->query('SELECT tb.id_produk, tb.judul, tb.lembar_saham AS saham_beli, tj.lembar_saham AS saham_jual, (tb.lembar_saham - tj.lembar_saham) AS sisa FROM ( SELECT produk.id_produk, ("beli") AS sisi, produk.judul, SUM(b.lembar_saham) as lembar_saham, b.status_approve FROM trx_dana_invest AS b LEFT JOIN trx_produk AS produk ON produk.id_produk = b.id_produk WHERE b.id_pengguna = "'.$id_pengguna.'" AND b.status_approve = "approve" AND produk.status_approve = "approve" GROUP BY produk.id_produk ) AS tb LEFT JOIN ( SELECT produk.id_produk, ("jual") AS sisi, produk.judul, SUM(j.lembar_saham) as lembar_saham, j.status_approve FROM trx_dana_invest_jual AS j LEFT JOIN trx_produk AS produk ON produk.id_produk = j.id_produk WHERE j.id_pengguna = "'.$id_pengguna.'" AND j.status_approve != "refuse" AND produk.status_approve = "approve" GROUP BY produk.id_produk ) AS tj ON tj.id_produk = tb.id_produk WHERE tb.lembar_saham - tj.lembar_saham > 0');
}

public function getPortfolioProduk($id_pengguna, $id_produk)
{
  return $this->db->query('SELECT tb.id_produk, tb.judul, tb.lembar_saham AS saham_beli, tj.lembar_saham AS saham_jual, (tb.lembar_saham - tj.lembar_saham) AS sisa FROM ( SELECT produk.id_produk, ("beli") AS sisi, produk.judul, SUM(b.lembar_saham) as lembar_saham, b.status_approve FROM trx_dana_invest AS b LEFT JOIN trx_produk AS produk ON produk.id_produk = b.id_produk WHERE b.id_pengguna = "'.$id_pengguna.'" AND b.id_produk = "'.$id_produk.'" AND b.status_approve = "approve" AND produk.status_approve = "approve" GROUP BY produk.id_produk ) AS tb LEFT JOIN ( SELECT produk.id_produk, ("jual") AS sisi, produk.judul, SUM(j.lembar_saham) as lembar_saham, j.status_approve FROM trx_dana_invest_jual AS j LEFT JOIN trx_produk AS produk ON produk.id_produk = j.id_produk WHERE j.id_pengguna = "'.$id_pengguna.'" AND j.id_produk = "'.$id_produk.'" AND j.status_approve != "refuse" AND produk.status_approve = "approve" GROUP BY produk.id_produk ) AS tj ON tj.id_produk = tb.id_produk WHERE tb.lembar_saham - tj.lembar_saham > 0');
}
