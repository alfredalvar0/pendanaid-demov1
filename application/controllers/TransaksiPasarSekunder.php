...
class TransaksiPasarSekunder extends CI_Controller {
	...
	public function prosesUpdate($id){
		...
		$dataExist = $this->M_transaksipasarsekunder->getDataProduk([
			'id_produk' => $idProduk
		]);

		if ($dataExist->num_rows() == 0) {
			$dataProduk['created_at'] = date('Y-m-d H:i:s');
			$result = $this->M_transaksipasarsekunder->insert($dataProduk);
		} else {
			$result = $this->M_transaksipasarsekunder->update($dataProduk, $idProduk);
		}
