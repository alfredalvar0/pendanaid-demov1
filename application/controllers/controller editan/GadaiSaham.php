<?php 
	...
	class GadaiSaham extends CI_Controller {
		function __construct(){
			parent::__construct();
	        $this->load->helper('url');
	        $this->load->model('M_gadaisaham');
		...
		{			
			...
			$data['dataGadaiSaham'] = $this->M_gadaisaham->select_dana($where);
			$this->load->view('admin/gadaisaham/list_data', $data);
		}
		...
		public function update($id) {
			...
			$data['dataGadaiSaham'] 	= $this->M_gadaisaham->select_dana($where)->row();
			$data['content'] = 'admin/gadaisaham/form_update';
			$this->load->view('admin/indexadmin',$data);
		}
		...
		public function prosesUpdate(){
			...
			$dataGadaiSaham= $this->M_gadaisaham->select_dana($where)->row();
			$date=date('Y-m-d H:i:s');
			$msg="";
			if($this->input->post('status_approve')=="approve"){
				$msg ="Pengajuan Gadai Saham kamu sebesar Rp. ".number_format($this->input->post('jumlah_dana'),0,".",".")." berhasil di approve pada tanggal ".$date;
			} else if($this->input->post('status_approve')=="pending"){
				$msg ="Pengajuan Gadai Saham kamu sebesar Rp. ".number_format($dataGadaiSaham->jumlah_dana,0,".",".")." di pending pada tanggal ".$date;
			} else if($this->input->post('status_approve')=="refuse"){
				$msg ="Pengajuan Gadai Saham kamu sebesar Rp. ".number_format($dataGadaiSaham->jumlah_dana,0,".",".")." di tolak pada tanggal ".$date;
			}
			...
			//if ($this->input->post('status_approve') == "") {
				...
				$result = $this->M_gadaisaham->insertpesan($dataPesan);
			//}

			$result = $this->M_gadaisaham->update($dataDana,$iddana);
			...
			if ($result > 0) {
				...
				$out['msg'] = '<p class="box-msg">
				      <div class="info-box alert-success">
					      ...
					      <div class="info-box-content" style="font-size:20px">
				        	Pengajuan Gadai Saham Berhasil Dirubah</div>
					  </div>
				    </p>';
			} else{
				$out['status'] = '';
				$out['msg'] = '<p class="box-msg">
				      <div class="info-box alert-error">
					      ...
					      <div class="info-box-content" style="font-size:20px">
				        	Pengajuan Gadai Saham Gagal Dirubah</div>
					  </div>
				    </p>';
			}
